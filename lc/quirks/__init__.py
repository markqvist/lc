...
class QuirkRegistry:
    ...
    def _load_user_quirks(self):
        if not self.config.loading["user_quirks"]: RNS.log("User quirk loading disabled, skipping", RNS.LOG_DEBUG)
        else:
            ...
            for quirk_file in quirks_dir.glob("*.py"):
                ...
                try:
                    module = None # TODO: Actually import quirk directly from file
                    if hasattr(module, "quirk_id"):
                        quirk_id = module.quirk_id
                        self.quirks[quirk_id] = module
