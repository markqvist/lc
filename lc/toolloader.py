...
class ToolLoader:
    ...
    def _load_tools(self) -> None:
        for tool_dir in self.tool_dirs:
            if not tool_dir.exists(): continue
            
            for item in tool_dir.iterdir():
                if item.is_file() and item.suffix == '.py':             self._load_tool_file(item)
                elif item.is_dir() and (item / '__init__.py').exists(): self._load_tool_package(item)
    ...
    def _load_tool_file(self, file_path: Path) -> None:
        ...
        try:
            ...
            if spec is None or spec.loader is None: raise ImportError(f"Could not load spec from {file_path}")
        ...
        except Exception as e: raise RuntimeError(f"Failed to load tool from {file_path}: {e}") from e
    ...
    def _load_tool_package(self, dir_path: Path) -> None:
        ...
        try:
            ...
            if spec is None or spec.loader is None: raise ImportError(f"Could not load spec from {init_file}")
        ...
        except Exception as e: raise RuntimeError(f"Failed to load tool package from {dir_path}: {e}") from e
    ...
    def get_toolkits(self) -> Dict[str, Toolkit]: return self.toolkits
